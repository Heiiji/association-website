...
module.exports = function (app, db, parser) {
    var Mediapart = setInterval(function() {
        let nbr = 0;
        console.log('synchronisation : Mediapart\n');
        parser.parseURL('https://www.mediapart.fr/articles/feed').then((feed) => {
            Thread.findOne({name: 'mediapart'}, (err, thread) => {
                if (err) {
                    console.log(err);
                    return ;
                } else {
